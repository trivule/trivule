...
- In previous versions, the `qv-name` attribute was used to replace the name on an `input` element if desired. Now, it is used to customize the name used in the error message.
  - If your `input` tag is:
    ```html
    <input class="input" type="text" name="age" />
    ```
    The error message would be something like: *The **age** field is required*.
  - However, if you specify the `qv-name` attribute with the value `"age condition"`, the message would become: *The **age condition** field is required*.
  - You can also specify the attribute via JavaScript code using the `attribute` property of the `QvInputParams`:
    ```javascript
    {
...
- Added two events: `qv.form.passes` and `qv.form.fails`. These events allow you to know when the form is valid or not.

#### QvForm Class

The `QvForm` class is responsible for applying live validation to an HTML form. It enables developers to create an instance of `QvForm` and interact with it to enable live validation on a form.
...
- `validateOnChange(fn?: CallableFunction)`: Registers an event listener for the "change" event on the container element. If a callback function is provided, it will be executed before Quickv handles the form validation.
  ```typescript
...
- `validateOnQvValidated(fn?: CallableFunction)`: Registers an event listener for the "qv.input.validated" event on the container element. If a callback function is provided, it will be executed before Quickv handles the form validation.
  ```typescript
...
- `onPasses(fn: EventCallback)`: Attaches an event listener to

 the "qv.form.passes" event. This event is triggered when the form passes validation.

- `observeChanges(fn?: EventCallback)`: Attaches an event listener to the "qv.form.updated" event. This event is triggered when the form is updated, reinitializing and running the QvInputs for the form. The provided function is called with the form instance as a parameter.
